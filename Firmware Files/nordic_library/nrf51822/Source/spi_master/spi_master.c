...
bool spi_master_tx(uint32_t *spi_base_address, uint16_t transfer_size, const uint8_t *tx_data)
{
    uint32_t counter = 0;
    uint16_t number_of_txd_bytes = 0;
    uint32_t SEL_SS_PINOUT;
    uint8_t rx_buffer_dump;
    /*lint -e{826} //Are too small pointer conversion */
    NRF_SPI_Type *spi_base = (NRF_SPI_Type *)spi_base_address;
    ...
    while(number_of_txd_bytes < transfer_size)
    {
        ...
        spi_base->EVENTS_READY = 0U;
        rx_buffer_dump = (uint8_t)spi_base->RXD;
        number_of_txd_bytes++;
    }
...
bool spi_master_rx(uint32_t *spi_base_address, uint16_t transfer_size, uint8_t *rx_data)
{
    uint32_t counter = 0;
